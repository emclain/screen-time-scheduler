...
1. **CK propagation lag during AFMT round trip**: CloudKit silent pushes typically land in 1--15s but can spike to minutes under server load. Two directions to cover:
   - **Request push (child → parent)**: child writes an `ExtensionRequest` to CloudKit; parent devices wake via `CKQuerySubscription` to show the action notification. If the push is delayed:
     ...
     - **Topology (b), parent iPhone only**: there is no reliable fallback. iOS background execution is opportunistic, not on-demand; nothing in the plan guarantees the app is foregrounded. The parent sees the notification whenever APNs eventually delivers the push. If the parent happens to open the app during the delay, a foreground refresh will pick up the pending request opportunistically, but this is coincidence, not a mechanism. Acknowledged as an open limitation of topology (b) -- see Open Risks.
...
8. **Topology (b) has no reliable fallback for delayed request pushes** -- if the parent runs iPhone-only and APNs delays a child's AFMT request push, the parent sees it whenever APNs eventually delivers. iOS background execution cannot be coerced into deterministic polling. This is a structural limitation of the iPhone-only topology; the mitigation is to run topology (a) with a parent Mac daemon if AFMT latency matters to the household.
